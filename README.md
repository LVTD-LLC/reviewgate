...
| `grounding` | object or null | Required before an eligible P0-P3 can block; contains a stable `semantic_key`, the checked claim, causal path, test assessment, exact evidence (`new` for current-head lines, `old` for deleted diff lines), related tests, P0-P1 reproduction/proof, and rereview fields. An automatic `fixed` transition requires the delta to delete exact prior `new` evidence and the resolution to check every added line in that replacement block. Findings grounded only in previously deleted lines remain open for an explicit disposition. |
