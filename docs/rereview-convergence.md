...
Still-open findings remain present even if a later model pass omits or rewrites them. Reviewer silence is never fix evidence. The reviewer must either emit the equivalent finding again or emit the same semantic identity with `resolution_disposition: fixed`, a non-empty `resolution_evidence_summary`, and exact evidence on a changed current-head line proving the prior failure is gone. ReviewGate records that transition only when the relevant file changed and the evidence fingerprint differs from the prior record.
