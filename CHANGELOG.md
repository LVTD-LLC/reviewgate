...
- Added a stable `reviewgate-agent-result/v1` contract and exact-head `reviewgate check` retrieval. Every review outcome now writes action outputs and uploads a bounded, SHA-named `reviewgate-agent-result` artifact with structured findings, thread IDs, angle errors, costs, and disposition history.
- Added `reviewgate disposition` for authenticated collaborators to submit `accepted`, `fixed`, `rejected_with_evidence`, `already_implemented`, `intentional_contract`, or `needs_human` as repository/PR/head-bound JSON. Rereviews validate the GitHub author and carry the exact submission into canonical finding history without Markdown scraping.
