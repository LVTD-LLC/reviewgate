...
| `grounding` | object or null | Required before an eligible P0-P3 can block; contains a stable `semantic_key`, the checked claim, causal path, test assessment, exact evidence (`new` for current-head lines, `old` for deleted diff lines), related tests, P0-P1 reproduction/proof, and rereview fields. An automatic `fixed` transition requires the delta to delete every prior `new` evidence location and the resolution to check every added line in each replacement block. A pure deletion is allowed when other changed current-head evidence validates the repair. Findings grounded only in previously deleted lines remain open for an explicit disposition. |
