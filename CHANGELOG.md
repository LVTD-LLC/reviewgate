...
- Added a bounded built-in fixed-string search over Git-tracked files for `deep: true` reviews when ripgrep is not installed, preserving ephemeral semantic context on the supported GitHub-hosted runner without adding a setup step or persistent index.
- Preserved semantic-context matches when a fast `rg` process exits between the stdout reader and process poll, preventing valid repository context from being reported as unavailable. Advanced the action runtime pin to `v0.8.1`.
...
- Replaced the navbar initials tile with the ReviewGate shield logo and made the homepage hero CTA copy a complete AI-agent setup prompt for the local CLI and GitHub Action, including an explicit OpenRouter model choice.
- Added a blinded, versioned replacement benchmark with 44 historical and seeded cases, public PR #53 golden defects, exact semantic-key adjudication, deterministic baseline-versus-candidate reports, rollout thresholds, rereview stability/convergence and resource metrics, an explicit non-publishing live BYOK mode, JSON schemas, and a keyless/networkless CI gate.
...
- Bound agent disposition receipts to writer-only commit-status attestations so GitHub Actions can verify the exact comment author, reviewed SHA, and payload digest even when its token redacts maintainer associations. Advanced the action runtime pin to `v0.4.5`.
